...
export class MerkleDistributionService {
  ...
  static computeHash(schedule: Schedule): Buffer {
    const web3 = new Web3()
    const hash = web3.utils.soliditySha3(schedule.index, `0x${schedule.address.toString('hex')}`, schedule.receivingAmount.toNumber(), schedule.sendingAmount.toNumber())
